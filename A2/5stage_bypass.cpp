...
struct FiveStageWithBypass : FiveStage {
    FiveStageWithBypass(std::ifstream &file) : FiveStage(file) {}
    ...
    void executeID(PipelineRegisters::IF_ID & IF_ID,
        PipelineRegisters::ID_EX & ID_EX, PipelineRegisters::EX_MEM & EX_MEM, PipelineRegisters::MEM_WB & MEM_WB) {
        if (IF_ID.cmmnd.name == "nop") {
            ID_EX.nop = true;
            return;
        }
        ID_EX.r2 = IF_ID.cmmnd.r2;
        int rval1 = registers[IF_ID.cmmnd.r1];
        int rval2 = registers[IF_ID.cmmnd.r2];
        if (IF_ID.cmmnd.r1 !=0 && !EX_MEM.nop && EX_MEM.reg_to_write == IF_ID.cmmnd.r1)
            rval1 = EX_MEM.alu_output;
        else if (IF_ID.cmmnd.r1 !=0 && !MEM_WB.nop && MEM_WB.reg_to_write == IF_ID.cmmnd.r1)
            rval1 = MEM_WB.mem_output;
        if (IF_ID.cmmnd.r2 !=0 && !EX_MEM.nop && EX_MEM.reg_to_write == IF_ID.cmmnd.r2)
            rval2 = EX_MEM.alu_output;
        else if (IF_ID.cmmnd.r2 !=0 && !MEM_WB.nop && MEM_WB.reg_to_write == IF_ID.cmmnd.r2)
            rval2 = MEM_WB.mem_output;
        ID_EX.nop = false;
        ID_EX.alu_input1 = rval1;
        if (two_reg_type.find(IF_ID.cmmnd.name) != two_reg_type.end() ||
            ...
            ID_EX.alu_input2 = rval2;
        ID_EX.write_data = rval2;
    ...
    void executeEX(PipelineRegisters::ID_EX & ID_EX,
        PipelineRegisters::EX_MEM & EX_MEM, PipelineRegisters::MEM_WB & MEM_WB) {
        if (ID_EX.nop) {
            EX_MEM.nop = true;
            return;
        }
        ...
        if (ID_EX.branch == 1 && EX_MEM.alu_output == 0)
            ...
            EX_MEM.branch_address = PC + 1;
        if (!MEM_WB.nop && MEM_WB.reg_to_write == ID_EX.r2 && ID_EX.r2!=0)
            EX_MEM.write_data = MEM_WB.mem_output;
        else
    ...
    void executeCommandsPipelined() {
        ...
        while (PC < commands.size() || !IF_ID.is_nop() || !ID_EX.nop ||
            ...
            executeEX(ID_EX, EX_MEM, MEM_WB);
            executeID(IF_ID, ID_EX, EX_MEM, MEM_WB);
            executeIF(IF_ID);
            bool stall = false;
            ...
            if (!IF_ID.is_nop() && !ID_EX.nop && ID_EX.mem_to_reg && (ID_EX.reg_to_write == IF_ID.cmmnd.r1 || (ID_EX.reg_to_write == IF_ID.cmmnd.r2 && IF_ID.cmmnd.name != "sw"))) {
                IF_ID.flush();
                stall = true;
            }
