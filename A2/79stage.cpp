...
struct SevenNineStageWithoutBypass : SevenNineStage {
    SevenNineStageWithoutBypass(std::ifstream &file) : SevenNineStage(file) {}
    ...
    void executeRR(PipelineRegisters::ID_RR & ID_RR, PipelineRegisters::RR_EX & RR_EX) {
        if (ID_RR.nop) {
            RR_EX.nop = true;
            return;
        }
        ...
        RR_EX.opc = ID_RR.opc;
        RR_EX.alu_input1 = registers[ID_RR.r1];
        RR_EX.alu_input2 = ID_RR.reg_dst ? ID_RR.imm : registers[ID_RR.r2];
        RR_EX.write_data = registers[ID_RR.r2];
    }

    void executeEXtoMEM(PipelineRegisters::RR_EX & RR_EX, PipelineRegisters::EX_MEM & EX_MEM) {
        if (RR_EX.nop) {
            EX_MEM.nop = true;
            return;
        }
    ...
    // return true if IF_IF uses data from registers that are not yet written
    bool dataHazard(PipelineRegisters::IF_ID & IF_IF,PipelineRegisters::IF_ID & IF_ID, PipelineRegisters::ID_RR & ID_ID, PipelineRegisters::ID_RR & ID_RR, PipelineRegisters::RR_EX & RR_EX) {
        if (IF_IF.is_nop())
            return false;
        if (!IF_ID.is_nop() && IF_ID.cmmnd.r3 != 0 && (IF_ID.cmmnd.r3 == IF_IF.cmmnd.r1 || IF_ID.cmmnd.r3 == IF_IF.cmmnd.r2))
            return true;
        if (!ID_ID.nop && ID_ID.reg_to_write != 0 && (ID_ID.reg_to_write == IF_IF.cmmnd.r1 || ID_ID.reg_to_write == IF_IF.cmmnd.r2))
            return true;
        // rest for lw POP operations
        if (!ID_RR.nop && ID_RR.mem_to_reg && (ID_RR.reg_to_write == IF_IF.cmmnd.r1 || ID_RR.reg_to_write == IF_IF.cmmnd.r2))
            return true;
        if (!RR_EX.nop && RR_EX.mem_to_reg && (RR_EX.reg_to_write == IF_IF.cmmnd.r1 || RR_EX.reg_to_write == IF_IF.cmmnd.r2))
            return true;
        return false;
    }
    ...
    void executeCommandsPipelined() {
        ...
        while (PC < commands.size() || !IF_IF.is_nop() || !IF_ID.is_nop() || !ID_ID.nop || !ID_RR.nop || !RR_EX.nop || !EX_MEM.nop || !MEM_MEM.nop || !MEM_WB.nop) {
            ...
            executeEXtoMEM(RR_EX, EX_MEM);
            executeRR(ID_RR, RR_EX);
            executeID2(ID_ID, ID_RR);
            executeID1(IF_ID, ID_ID);
            executeIF2(IF_IF, IF_ID);
            executeIF1(IF_IF);
            bool stall = false;
            if (!IF_IF.is_nop() && !IF_ID.is_nop() && offset_type.find(IF_ID.cmmnd.name) != offset_type.end() && offset_type.find(IF_IF.cmmnd.name) == offset_type.end()) {
                IF_IF.flush();
                stall = true;
            }
            if (!IF_IF.is_nop() && !ID_ID.nop && (three_reg_type.find(IF_IF.cmmnd.name)!=three_reg_type.end() || two_reg_type.find(IF_IF.cmmnd.name)!=two_reg_type.end()) && ID_ID.mem_to_reg) {
                IF_IF.flush();
                stall = true;
            }
            // jump stalls
            ...
            if (dataHazard(IF_IF, IF_ID, ID_ID, ID_RR, RR_EX)) {
                IF_IF.flush();
                stall = true;
            }
            if (!stall)
...
int main(int argc, char ** argv) {
    ...
    SevenNineStageWithoutBypass mips(file);
